...
def load_knowledge_base(uploaded_file: UploadedFile) -> str:
    ...
    # STUDENT CODE HERE
    try:
        uploaded_file.seek(0)    #sets the cursor to the beginning of the csv file so it doesn't read from the end of the file      
        df = pd.read_csv(uploaded_file)  #reads the uploaded file
        for col in REQUIRED_CSV_COLUMNS:
            if col not in df.columns:
                return f"Error: CSV is missing required column '{col}'"
        if df.empty:                          
            return "Error: The CSV file is empty, no data found"
        parts = []                        
        for index, row in df.iterrows():
            text = f"Topic: {row['topic']}\nInformation: {row['information']}"
            parts.append(text)
        return "\n\n".join(parts)             
    except Exception as e: #Exception is used so that the app doesn't crash and instead returns the erros so that we can fix it
        return f"Error: {e}"
...
def build_prompt(
    ...
    # STUDENT CODE HERE
    return f"""
    You are a {domain} assitant.
    Domain constraint: Only answer{domain} question.
    Knowledge Base:{knowledge_base}
    Style Guidelines: Tone:{tone}, Length:{length}, Audience:{audience}
    User Question: {user_question}
    Give answers based only on the knowledge base above"""

#the above code is a prompt to send to OpenAI
#It asks the Ai to answer questions related to the domain, tells it how it should reply the tone, length, audience from the knowledge base provided
...
def get_ai_response(prompt: str) -> str:
    ...
    if not api_key:
        return "API Key not found, please Enter your OpenAI key in the sidebar"
    try:
        client=OpenAI(api_key=api_key) #this create a connection to OpenAI using the key
        ...
        return content if content else "Error: Empty response from AI."
...
def render_setup_tab() -> None:
    ...
    # STUDENT CODE HERE
    st.header("Setup")
    selected = st.radio("Pick a Domian:", AVAILABLE_DOMAINS, key="setup_domain")
    ...
    if uploaded_file:
        ...
        if result.startswith("Error:"):
            ...
            st.session_state.knowledge_base = result #this saves the text from the csv so when we go to the chat tab it can access the same text
            st.session_state.uploaded_filename = uploaded_file.name
            st.success(f"File uploaded: {uploaded_file.name}")
            with st.expander("Show the Knowledge Base"):
                st.text(result)
...
def render_chat_tab() -> None:
    ...
    with st.expander("Response style options"):
        ...
        length=col2.selectbox("Length", LENGTH_OPTIONS, key="length")
        audience=col3.selectbox("Audience", AUDIENCE_OPTIONS, key="audience")
...
def render_quick_questions_tab() -> None:
    ...
    if not is_setup_complete():
        return
    st.caption("Click a question then switch to Chat")
    domain=st.session_state.selected_domain
    questions=PREBUILT_QUESTIONS.get(domain,[])
    def select_question(q:str)->None:
        st.session_state.chat_questions=q
